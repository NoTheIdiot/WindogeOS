...
void init_syscall(void) {
    log("init_syscall: enter");
    uint64_t top = (uint64_t)(kernel_stack + sizeof(kernel_stack));
    top &= ~0xFULL;
    per_core_gs.kernel_stack_top = top;
    per_core_gs.user_rsp_scratch = 0;
    log("init_syscall: kernel_stack_top set");

    wrmsr(IA32_KERNEL_GS_BASE, (uint64_t)&per_core_gs);
    log("init_syscall: wrote IA32_KERNEL_GS_BASE");

    uint64_t efer = rdmsr(IA32_EFER);
    efer |= (1 << 0);
    wrmsr(IA32_EFER, efer);
    log("init_syscall: EFER updated");

    uint64_t star = ((uint64_t)0x10 << 32) | ((uint64_t)0x08 << 48);
    wrmsr(IA32_STAR, star);
    log("init_syscall: STAR written");

    wrmsr(IA32_LSTAR, (uint64_t)syscall_entry);
    log("init_syscall: LSTAR written");
    wrmsr(IA32_FMASK, 0x200);
    log("init_syscall: FMASK written");
    log("init_syscall: exit");
}
...
uint64_t c_syscall_handler(
    ...
    switch (sys_id) {
        ...
        default:
            log("c_syscall_handler: default");
            return (uint64_t)-1;
    }

    log("c_syscall_handler: exit");
    return (uint64_t)-5;
}
