...
uint8_t time_read_rtc(int reg) {
    lowlevel_ports_outb(0x70, reg);
    return lowlevel_ports_inb(0x71);
}
...
void time_rtc_init(void) {
    ...
    uint8_t prev = lowlevel_ports_inb(0x71);
    lowlevel_ports_outb(0x70, 0x8A);
...
void time_show() {
    ...
    if (time.hour < 10) dogeio_text_print("0");
    string_itoa(time.hour, buffer);
    dogeio_text_print(buffer);
    dogeio_text_print(":");

    if (time.minute < 10) dogeio_text_print("0");
    string_itoa(time.minute, buffer);
    dogeio_text_print(buffer);
    dogeio_text_print(":");

    if (time.second < 10) dogeio_text_print("0");
