...
static size_t read_token(const char *text, char *token, size_t token_size) {
    if (text == NULL || token == NULL || token_size == 0) return 0;
    size_t length = 0;
    skip_spaces(&text);
...
static int parse_number(const char *text, uint64_t *value) {
    ...
    while (text[index] != '\0' && text[index] != ' ' && text[index] != '\t' && text[index] != '\r' && text[index] != '\n') {
        uint8_t digit;
...
static int parse_value(const char *text, uint64_t *value) {
    ...
    if (*text == '$') {
        text++;
        char var_name[16];
        if (read_token(text, var_name, sizeof(var_name)) == 0) return 0;
        variable *found = get_variable(var_name);
...
int system_dogescript_execute(const char* script_text) {
    ...
    for (size_t steps = 0; line != NULL && *line != '\0' && steps < DOGESCRIPT_MAX_STEPS; steps++) {
        ...
        if (str_startswith(text, "PRINT ")) {
            ...
            if (*text == '"') {
                text++;
                while (*text != '"' && *text != '\0') {
                    dogeio_text_printchar(*text++);
                }
            } else if (*text == '$') {
                text++;
                char var_name[16];
                if (read_token(text, var_name, sizeof(var_name)) > 0) {
                    variable *found = get_variable(var_name);
                    if (found != NULL) {
                        if (found->is_string) {
                            dogeio_text_print(found->string_value);
                        } else {
                            print_number(found->value);
                        }
                    } else {
                        dogeio_text_print("(undefined)");
                    }
                }
            } else {
                uint64_t value;
        ...
        if (str_startswith(text, "VAR ")) {
            ...
            if (assignment != NULL && read_token(text + 4, name, sizeof(name)) > 0) {
                ...
                if (*assignment == '"') {
                    ...
                    assignment++;
                    while (*assignment != '"' && *assignment != '\0' && value_length + 1 < sizeof(value_text)) {
                        value_text[value_length++] = *assignment++;
                    }
                    value_text[value_length] = '\0';
                    set_variable(name, 0, value_text, true);
                } else {
                    uint64_t val;
                    if (parse_value(assignment, &val)) {
                        set_variable(name, val, NULL, false);
                    }
                }
            }
            line = next_line(line);
        ...
        if (str_startswith(text, "OUTB ")) {
            ...
            if (parse_token_value(text + 5, &port)) {
                ...
                while (*second != ' ' && *second != '\t' && *second != ',' && *second != '\0') second++;
                skip_spaces(&second);
                if (*second == ',') second++;
                if (parse_token_value(second, &val)) {
                    ports_outb((uint16_t)port, (uint8_t)val);
                }
            }
            line = next_line(line);
            continue;
        ...
        if (str_startswith(text, "INB ")) {
            ...
            if (parse_token_value(text + 4, &port)) {
                ...
                if (arrow != NULL) {
                    char name[16];
                    if (read_token(arrow + 2, name, sizeof(name)) > 0) {
                        uint8_t result_val = ports_inb((uint16_t)port);
                        set_variable(name, result_val, NULL, false);
                    }
                }
            }
            line = next_line(line);
