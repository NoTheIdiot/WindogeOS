...
if args.arch == "x86_64":
    qemu_cmd = f"qemu-system-x86_64 -rtc base=localtime -drive format=raw,file={img_file},if=ide -M q35 -m 256M"
elif args.arch in ["arm64", "aarch64"]:
    qemu_cmd = f"qemu-system-aarch64 -rtc base=localtime -M virt -cpu cortex-a57 -m 256M -bios limine-binary/BOOTAA64.EFI -drive format=raw,file={img_file},if=none,id=drv0 -device virtio-blk-device,drive=drv0"
else:
